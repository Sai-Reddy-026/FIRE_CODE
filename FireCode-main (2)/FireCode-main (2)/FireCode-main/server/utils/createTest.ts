...
async function executeSingleTestCase(
    ...
    try {
        ...
        let errorMessage = data.compile_output || data.stderr || undefined;

        let userOutput = data.stdout ? data.stdout.trim() : "";
        const expectedOutput = testCase.expectedOutput.trim();
...
export async function runOfficialSolutionOnInput(
    ...
    try {
        ...
        const stderr = data.compile_output || data.stderr || undefined;
        const runtime = Math.round((data.time ? parseFloat(data.time) : 0) * 1000);
